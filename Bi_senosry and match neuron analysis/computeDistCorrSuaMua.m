% compute singleuntis distance correlation between singleunits and
% multiunits psth.   %TODO add 
...
t_samp =  sua_units_lin(1).ves.time>0 & sua_units_lin(1).ves.time<1.5;
...
for i=1:sua_nunits_lin
    ...
    for j=1:nconds
        resp_sua_lin(i,j,:) = mean(sua_units_lin(i).(cond{j}).rate_pst(:,t_samp)); % just motion period
    end
end
%extract multiunits
for i=1:mua_nunits_lin
    ...
    for j=1:nconds
        resp_mua_lin(i,j,:) = mean(mua_units_lin(i).(cond{j}).rate_pst(:,t_samp)); % just motion period
    end
end
...
interDist =[0:0.1:1.5];
figure; hold on;
plot(interDist,corr_sua_mua_pairs_mu_lin(1,:),'-r', 'LineWidth',2);
plot(interDist,corr_sua_mua_pairs_mu_lin(2,:),'-g','LineWidth',2);
plot(interDist,corr_sua_mua_pairs_mu_ang(1,:),'--r','LineWidth',2);
plot(interDist,corr_sua_mua_pairs_mu_ang(2,:),'--g','LineWidth',2);
box off;
set(gca, 'TickDir', 'out', 'ylim',([0 0.35]), 'ytick', [0 0.1 0.2 0.3], 'FontSize',18)
xlabel('SUA-MUA distance (mm)'); ylabel('Temporal corr between SUA-MUA');
