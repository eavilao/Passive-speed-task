...
%add by JL 03/06/2015 for loadind Plxon data
sName = ['Z:\Data\MOOG', PATH(fa(3):fa(4)), 'Uprobe\', FILE(1:length(FILE) - 4),'.mat']; 

if (exist(sName,'file'))
   disp('Loading Plexon sorted data!')
   cbHandle = findobj(gcbf, 'Tag', 'Sync Pulses');
   if cbHandle.Value
       [good_data SortedChannel]=LoadPlxData_SynPluse(good_data,sName,good_trials);
   else
       [good_data SortedChannel]=LoadPlxData(good_data,sName);%get the Spike2 sorted data
   end;  
   num_recorded_spike_channels=SortedChannel;
end
%end 03/05/2015
