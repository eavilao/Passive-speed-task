...
SLANT_DISCRIMINATION = 199; % FOR Ari's protocol
VISUAL_MOTION_PURSUIT_GALVO_LOWSPEED = 201; %
VISUAL_MOTION_PURSUIT_GALVO_HIGHSPEED = 127; %,   //202
AZ_TUNUNG_W_PURSUIT_VISUAL = 128; %     //203
AZ_TUNING_W_HEAD_PURSUIT = 129; %   //204
AZ_TUNUNG_W_PURSUIT = 130;  %   //205 
ROTATION_HEADING_DISCRIM_YAW = 134;  %Jing 10/18/2012
PURSUIT_TRAP = 138;
VARYFIX_HEADING_DISCRIM = 198;    %Jing 11/05/2012
YAW_ROT_FIXATION = 140;
FINE_ROTATION_DISCRIM = 141; 
HEADING_PURSUIT_DISCRIM = 142;  % SL add 
HEADING_DISCRIM_GABOR = 143;    %Jing 10/19/2016
...
protocol_names(142) = {
    'MOOG: Fine ROTATION DISCRIMINATION'     
};%added by Jing for Fine Rot Discrm.

protocol_names(199) = {
    'MOOG: Varying FP HEADING DISCRIM'   
};%added by Jing 11/05/2012

protocol_names(200) = {
    'SLANT DISCRIMINATION'     
};%added by Tunde for Ari

protocol_names(144) = {
    'MOOG: GABOR HEADING DISCRIMINATION'     
};%added by Jing 10/19/2016
...
protocol_names(140) = {'Plot YAW_ROT_FIXATION'}; % added this because of Jing's pursuit protocol --- Tunde

protocol_names(143) = {'Plot Heading Pursuit  Discrimination'}; % added this because of Jing's pursuit protocol --- Tunde
...
moog_keywords{num_keys}='TRANS_ROT_TYPE';       TRANS_ROT_TYPE=num_keys; 		num_keys=num_keys+1;   %%%%%%   for head pursuit only, added by Baowang at 10/15/14
...
moog_keywords{num_keys}='EYE_HEAD_MODE';		EYE_HEAD_MODE=num_keys; 		num_keys=num_keys+1;   %%%%%%   for head pursuit only, added by Baowang at 10/15/14
...
%% ??
moog_keywords{num_keys}='ROT_VELOCITY'; ROT_VELOCITY=num_keys; num_keys=num_keys+1;
moog_keywords{num_keys}='TRAP_RAMP_DUR'; TRAP_RAMP_DUR=num_keys; num_keys=num_keys+1;
%% add by sl
moog_keywords{num_keys}='PURSUIT_STIM_TYPE'; PURSUIT_STIM_TYPE=num_keys; num_keys=num_keys+1;
%%Jing 10/19/2016
moog_keywords{num_keys}='PHASE'; PHASE=num_keys; num_keys=num_keys+1;
...
analysis_strings{YAW_ROT_FIXATION + 1} = ...
    {   'Plot YAW_ROT_FIXATION'};
...
analysis_strings{PURSUIT_TRAP + 1} = ...
    {    'Plot PURSUIT Eye Track',
        'Plot PURSUIT_TRAP',
        'Plot PURSUIT_TRAP_training',
};
...
analysis_strings{HEADING_DISCRIM + 1} = ...
    {   'Plot Psycho_neuro_cum',
        ...
        'Plot Psychometric (Kalpana)',
        'Plot Psychometric',  
        'Plot Psychometric (Two-target)',
...
analysis_strings{AZ_TUNING_W_HEAD_PURSUIT + 1} = ...
    {	'AZ_TUNING_W_HEAD_PURSUIT'
}; %added by Tunde
...
analysis_strings{SLANT_DISCRIMINATION + 1} = ...
    {	'Plot Psychometric (Ari)'
}; %added by Tunde

analysis_strings{ROTATION_HEADING_DISCRIM_YAW + 1} = ...
    {	'Plot Psychometric (Rot)'
        'Plot Psychometric (Rot, Two-Target)'   %Jing 12/17/2012
        'Plot Psychometric (Rot, Courtney)'     %Jing 08/12/2013
}; %Jing 10/18/2012

analysis_strings{FINE_ROTATION_DISCRIM + 1} = ...
    {	'FineRotDiscrm (Psycho)'
        'FineRotDiscrm (Neuron)'
        'FineRotDiscrm  afferent (Two-target)'  
}; %Jing 08/29/2013



analysis_strings{HEADING_PURSUIT_DISCRIM + 1} = ...
    {	'Plot Psychometric_eye_pursuit',
        'Plot eye_pursuit_HD_eyemovement',
}; %Added by Baowang 01/23/2015
     

analysis_strings{VARYFIX_HEADING_DISCRIM + 1} = ...
    {	'Plot Psychometric (Varying FP)'
        'Plot Psychometric (Varying FP, Two-Target)'
}; %Jing 11/05/2012

analysis_strings{HEADING_DISCRIM_GABOR + 1} = ...
    {	'Plot Psychometric (Gabor)',  
        'Plot Psychometric (Gabor,Two-target)',
}; %Jing 10/19/2016
