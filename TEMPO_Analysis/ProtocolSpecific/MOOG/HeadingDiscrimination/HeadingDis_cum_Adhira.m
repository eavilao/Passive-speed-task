...
% fit_data_psycho_cum{1}(:, 2)
% fit_data_psycho_cum{2}(:, 2)
% fit_data_psycho_cum{3}(:, 2)
...
% for i = 1:9
%     respves(1,1+repetition*(i-1):repetition+repetition*(i-1))=resp{1,i}(1:repetition);
%     vesheading(1,1+repetition*(i-1):repetition+repetition*(i-1)) = unique_heading(i);
%     [rr,pp] = corrcoef(vesheading, respves);
%     rves = rr(1,2);
%     pves = pp(1,2);
%     
%     respvis(1,1+repetition*(i-1):repetition+repetition*(i-1))=resp{2,i}(1:repetition);
%     [rr,pp] = corrcoef(vesheading, respvis);
%     rvis = rr(1,2);
%     pvis = pp(1,2);
%     
%     [rr,pp] = corrcoef(respves, respvis);
%     rvesvis = rr(1,2);
%     pvesvis = pp(1,2);
% end
...
%buff = sprintf(sprint_txt, FILE, Thresh_psy{:}, Thresh_neu{:}, CP_all{:}, p{:} );
