...
for k = 1 : length(unique_stim_type)
    for c = 1:length(unique_motion_coherence)
        ...
        if isempty(find(unique_heading == 0))
...
%             fit_data_neuro_cum{c,k}(i,3) = sum(trials_n);  % for later function fit use
            tmptrial = sum(trials_n);  % for later function fit use
...
%        fit_data_neuro_cum{c,k}(:,1) = unique_heading( unique_heading~=unique_heading(length(unique_heading)/2) & unique_heading~=unique_heading(1+length(unique_heading)/2) );
         ...
         fit_data_neuro_cum{c,k}(:,3) = tmptrial; 
...
% 
% xlswrite('z:\users\sheng\m43c30r4_A', [ unique_heading   resp_mat{1,k}(:) resp_mat_err{1,k}(:) ]);
% 
% xlswrite('z:\users\sheng\m43c30r4_C_neu',   [xi', cum_gaussfit(neu_perf{1,k}, xi)']);
% xlswrite('z:\users\sheng\m43c30r4_C_neu',  [neu_heading, Neuro_correct{1,k}'],  'C1:D8');
% 
% xlswrite('z:\users\sheng\m43c30r4_C_psy',   [ xi', cum_gaussfit(psy_perf{1,k}, xi)']);
% xlswrite('z:\users\sheng\m43c30r4_C_psy',  [unique_heading, psycho_correct{1}(k,:)'],  'C1:D8');
% 
...
% m_count = [m_count  m_count1 m_count2(:,2)]
% xlswrite('z:\users\sheng\m43c30r4_B',   m_count);
% 
% 
% xlswrite('z:\users\sheng\m43c30r4_D',   count);
