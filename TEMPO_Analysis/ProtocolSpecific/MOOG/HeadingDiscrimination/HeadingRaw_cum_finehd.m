...
function HeadingRaw_cum_finehd(data, Protocol, Analysis, SpikeChan, StartEventBin, StopEventBin,StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE);
    ...
    for c = 1:length(unique_motion_coherence)
        ...
        for i = 1:length(unique_heading)
          ...
          %  if (length(resp_left_choose{k,i}) / length(resp{k,i}) <0.25) |  (length(resp_left_choose{k,i}) / length(resp{k,i}) >0.75)  
            ...
            if i == 1 ||  i == length(unique_heading)
                Z_Spikes_dec(select) = 9999;   % similar to NaN, just make a mark            
            end 
        end  
        % now across all data 
        ...
        resp_left_dec = Z_Spikes_dec( (motion_coherence == unique_motion_coherence(c)) & (stim_type == unique_stim_type(k)) & (choice == LEFT) & (Z_Spikes~=9999) ); 
        resp_right_dec = Z_Spikes_dec( (motion_coherence == unique_motion_coherence(c)) & (stim_type == unique_stim_type(k)) & (choice == RIGHT) & (Z_Spikes~=9999) ); 
    end
end
    ...
    for c = 1:length(unique_motion_coherence)
        ...
        for i = 1 : length(unique_heading)  
            ...
            if (length(resp_left_all{c,k}) > 3) & (length(resp_right_all{c,k}) > 3)
             ...
             if (length(resp_left_dec) > 3) & (length(resp_right_dec) > 3)
                CP_dec = rocN( resp_left_dec,resp_right_dec,100 );
            else
                CP_dec = NaN; 
            end
            if  line_re{c,k} > 0
...
%        fit_data_neuro_cum{c,k}(:,1) = unique_heading( unique_heading~=unique_heading(length(unique_heading)/2) & unique_heading~=unique_heading(1+length(unique_heading)/2) );
       ...
       wichman_dec = pfit(fit_data_neuro_cum{c,k}(2:end-1,:),'plot_opt','no plot','shape','cumulative gaussian','n_intervals',1,'sens',0,'compute_stats','false','verbose','false');
       Bias_dec = wichman_dec.params.est(1);
       Thresh_dec = wichman_dec.params.est(2);
...
%         Thresh_neu{c,k} = ttt;
        ...
        if Thresh_dec <0  |  Thresh_dec > 300
            Thresh_dec = 300;
            Bias_dec = 300;
        end
...
for k = 1:length(unique_stim_type) 
    for c = 1:length(unique_motion_coherence)
        ...
        for n = 1 : perm_num
            ...
            resp_left_all_dec = Z_Spikes_perm( (motion_coherence == unique_motion_coherence(c)) & (stim_type == unique_stim_type(k)) & (choice == LEFT) & (Z_Spikes~=9999) ); 
            resp_right_all_dec = Z_Spikes_perm( (motion_coherence == unique_motion_coherence(c)) & (stim_type == unique_stim_type(k)) & (choice == RIGHT) & (Z_Spikes~=9999) ); 
            ...
            CP_all_perm_dec(n) = rocN( resp_left_all_dec, resp_right_all_dec,100 );
            
            if  line_re{c,k} > 0  
                CP_all_perm{c,k}(n) = 1 - CP_all_perm{c,k}(n); 
                CP_all_perm_dec(n) = 1 - CP_all_perm_dec(n); 
            end  
        ...
        if (length(resp_left_all{c,k}) > 3) & (length(resp_right_all{c,k}) > 3) 
         ...
         % now calculate p value or significant test with less heading
            hist_perm = [];
            hist_perm = hist( CP_all_perm_dec(:), x_bin );  % for permutation
            bin_sum = 0;
            n = 0;
            while ( n < (CP_dec/bin) )
                 n = n+1;
                 bin_sum = bin_sum + hist_perm(n);
                 if CP_dec > 0.5                  % note it's two tail test
                    p_dec = 2*(perm_num - bin_sum)/ perm_num;    % calculate p value for CP_all
                 else
                    p_dec = 2* bin_sum / perm_num;
                 end
            end
...
for k = 1:length(unique_stim_type)
 ...
 %   plot data in logarithmical space instead of linspace
    ...
    hold on;
    xi_dec = unique_heading(2) : 0.1 : unique_heading(end-1);
    plot( xi_dec, cum_gaussfit([Bias_psy_dec  Thresh_psy_dec], xi_dec),  'r' );
...
%    xi = unique_heading(2):0.1:unique_heading(8);
    ...
    hold on;
    plot( xi_dec, cum_gaussfit([Bias_dec  Thresh_dec], xi_dec),  'r' );
...
text(0,6, 'Less');
text(20,6, num2str(Bias_dec , '%.2f') );
text(27,6, num2str(Thresh_dec , '%.2f') );

text(5,6, num2str(Bias_psy_dec , '%.2f') );
text(12,6, num2str(Thresh_psy_dec , '%.2f') );
...
for k = 1:length(unique_stim_type)
    ...
    text(20,8-k,num2str(Bias_neu{1,k} , '%.2f'));
    text(27,8-k,num2str(Thresh_neu{1,k} , '%.2f'));
    
     text(35,8-k,num2str( CP{c,k}(midpt)', '%.2f') ); 
     text(42,8-k,num2str(p_a(c,k), '%.2f') ); 

     text(49,8-k,num2str(CP_all{1,k}', '%.2f') ); 
     text(58,8-k,num2str(p{1,k}', '%.2f') );   
...
% xlswrite('z:\users\sheng\m43c30r3_A', [ flipud(unique_heading)   resp_mat{1,k}(:) resp_mat_err{1,k}(:) ]);
% 
% xlswrite('z:\users\sheng\m43c30r3_C_neu',   [xi', cum_gaussfit(neu_perf{1,k}, xi)']);
% xlswrite('z:\users\sheng\m43c30r3_C_neu',  [neu_heading, Neuro_correct{1,k}'],  'C1:D8');
% 
% xlswrite('z:\users\sheng\m43c30r3_C_psy',   [ xi', cum_gaussfit(psy_perf{1,k}, xi)']);
% xlswrite('z:\users\sheng\m43c30r3_C_psy',  [unique_heading, psycho_correct{1}(k,:)'],  'C1:D8');
% 
% addpath('Z:\Users\sheng\program\newtool');
% m_count1 =  Docount(resp{1}', 50,  100, 5);
% m_count2 =  Docount(resp{9}', 50,  100, 5);
% m_count = [m_count1 m_count2(:,2)];
% 
% m_count1 =  Docount(resp{2}', 50,  100, 5);
% m_count2 =  Docount(resp{8}', 50,  100, 5);
% m_count = [m_count  m_count1 m_count2(:,2)]
% 
% m_count1 =  Docount(resp{3}', 50,  100, 5);
% m_count2 =  Docount(resp{7}', 50,  100, 5);
% m_count = [m_count  m_count1 m_count2(:,2)]
% 
% m_count1 =  Docount(resp{4}', 50,  100, 5);
% m_count2 =  Docount(resp{6}', 50,  100, 5);
% m_count = [m_count  m_count1 m_count2(:,2)]
% xlswrite('z:\users\sheng\m43c30r3_B',   m_count);
% 
% 
% xlswrite('z:\users\sheng\m43c30r3_D',   count);
