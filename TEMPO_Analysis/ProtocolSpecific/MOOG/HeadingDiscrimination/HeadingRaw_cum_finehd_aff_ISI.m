...
%             raster(i)
            if  isempty( sel_left )
                ...
                for jjj = 1 : length(sel_left)
                    raster_left{i, jjj} =  data.spkdt{1, sel_left(jjj)};
                end
            end
            
           if  isempty( sel_right )
                raster_right{i, 1} = -999;
            else
                for jjj = 1 : length(sel_right)
                    raster_right{i, jjj} =  data.spkdt{1, sel_right(jjj)};
                end
            end
...
% return;
%% draw ISI
addpath('C:\ROC_code\new_tool\distribution\regress_6');
[temp, Files] =  xlsread('C:\google1\ISI.xls');
for ii = 1 : length(Files)
    if  strcmp( strtrim(char(Files(ii))),    FILE(1:end-4))  == 1
        axisval = temp(ii, :);
    end
end
...
xx =[]; yy = [];
    ...
    for ii = 1 : size(raster_left, 2)
      if  length(raster_left{jjj,1})  == 1
          if raster_left{jjj,1} == -999
              continue;
          end
      end
        
        leftspk = raster_left{jjj, ii};

        if ~isempty(leftspk)
          plot(diff(leftspk(1:end-1))*1000, diff(leftspk(2:end))*1000, 'r.' );
          hold on;
          xx = [xx; diff(leftspk(1:end-1))*1000];
          yy = [yy; diff(leftspk(2:end))*1000];
        end
    end
           if ~isempty(xx)
               index = find(xx < axisval(2)  &  xx > axisval(1) & yy < axisval(2)  &  yy > axisval(1) );
               
           try,
             [a, b]  = regress_perp(xx(index), yy(index));
           catch,
              [aa, bb]  = regress(yy(index),  [ones(length(xx(index)),1)  xx(index)]) ;
              a = aa(2);
              b = aa(1);
           end
            xxx = min(xx)-1 : 0.1 :  max(xx)+1;
            yyy = xxx*a+b;
             plot(xxx, yyy, 'r');  
           end
           [a b]
      xx =[]; yy = [];
     for ii = 1 : size(raster_right, 2)
         try,
          if  length(raster_right{jjj,1})  == 1
                if raster_right{jjj,1} == -999
                    continue;
                end
          end
         catch,
        ...
        rightspk = raster_right{jjj, ii};
        if ~isempty(rightspk)
          plot(diff(rightspk(1:end-1))*1000, diff(rightspk(2:end))*1000, 'b.' );
          hold on;
          xx = [xx; diff(rightspk(1:end-1))*1000];
          yy = [yy; diff(rightspk(2:end))*1000];
          ...
          index = find(xx < axisval(2)  &  xx > axisval(1) & yy < axisval(2)  &  yy > axisval(1) );
            try,
             [a, b]  = regress_perp(xx(index), yy(index));
              ...
              [aa, bb]  = regress(yy,  [ones(length(xx),1)  xx]) ;
              a = aa(2);
              b = aa(1);
           end
            xxx = min(xx)-1 : 0.1 :  max(xx)+1;
    ...
    if jjj == 1
      title([FILE(1:end-4) '    ' num2str(unique_heading(jjj), '%.1f')     ]);
    else
     title([num2str(unique_heading(jjj), '%.1f')    ]);
    end
