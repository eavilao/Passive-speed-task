...
function HeadingRaw_cum_finehd_aff(data, Protocol, Analysis, SpikeChan, StartEventBin, StopEventBin,StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE);
...
if strcmp(FILE, 'm24c420r1.htb') == 1
    CP{c,k}(midpt) = 0.27;
    p_a(c,k) = 0.27;
end
...
for k = 1:length(unique_stim_type)
    ...
    text(35,8-k,num2str(Thresh_neu{1,k} , '%.2f'));
     ...
     text(47,8-k,num2str( 1-CP{c,k}(midpt)', '%.2f') ); 
     text(55,8-k,num2str(p_a(c,k), '%.2f') ); 

     text(67,8-k,num2str(1-CP_all{1,k}', '%.2f') ); 
...
resp_left = resp_right_all{1,1};
resp_right = resp_left_all{1,1};
...
resp_left = resp_right_choose{1,1, midpt};
resp_right = resp_left_choose{1,1, midpt};
