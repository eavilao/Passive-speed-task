function  [x_time_smooth,  m_PSTH]  = Heading_PSTH2_aff(data, Protocol, Analysis, SpikeChan, StartEventBin, StopEventBin,StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE,  FigureIndex);
...
% for ii = 1 : size(xx, 1)
%     for jj = 1 : 40
%        yy(ii, jj) =   sum( xx(ii, 1000+ (jj-1)*25 -100 : 1000+ (jj-1)*25 +100 )  )*10/4;
%     end
% end
%  plot(mean(yy(:,1:28)), 'Color',  Rainbow(kk, :))
% end
...
SpikeArray
% figure;
%        for jj  = 1 : size(SpikeArray{1}, 1)
%            temp = SpikeArray{9};
%            for kk = 1 : 80
%                  xx(jj,kk) = sum(temp(jj, 996-500-12 + (kk-1)*25 :996-500+12 + (kk-1)*25  ))*40;
%            end
%            half_wind = 4;
%            g = normpdf(-half_wind, 0 ,  half_wind );
%            fil = g/sum(g);
%            yy = filter(fil, 1,  xx(jj,:));
%            plot(yy, 'Color',  Rainbow(jj, :));
%            hold on;
%        end
%        
%        figure;
...
minpsth = min(min(m_PSTH(:, 32 :80)));
maxpsth = max(max(m_PSTH(:, 32 :80)));
...
saveas(gcf, ['Z:\Users\sheng\aferent\'   FILE(1:end-4)],   'png');
close(gcf);
