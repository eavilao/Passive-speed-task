function  [x_time_smooth,  m_PSTH]  = Heading_PSTH_pursuit(data, Protocol, Analysis, SpikeChan, StartEventBin, StopEventBin,StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE,  FigureIndex);
...
binsize=50;
Step=25;%Step=100;
WindowInterval=300; %timebin for plot PSTH
...
trials_per_rep = length(unique_elevation)*length(unique_stim_type);
...
for k=1:1 
    ...
    for i=1:length(unique_elevation)
        clear select; select = logical( (elevation==unique_elevation(i)) & (stim_type==unique_stim_type(k)) );  
...
for k=1:1  
    for i=1:length(unique_elevation)
        ...
        str{i}  =  num2str(unique_elevation(i),  '%d');         
...
figure(10);
axes('position',[0.66 0.3, 0.32 0.45]);
