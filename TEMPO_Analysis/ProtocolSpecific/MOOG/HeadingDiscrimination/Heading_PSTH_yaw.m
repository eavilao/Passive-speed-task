function  [x_time_smooth,  m_PSTH]  = Heading_PSTH2(data, Protocol, Analysis, SpikeChan, StartEventBin, StopEventBin,StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE,  FigureIndex);
...
%         end
        str{i}  =  num2str([unique_heading(i)],  '%.2f'); 
...
delay = 0.1
figure(10);
axes('position',[0.36 0.05, 0.4 0.35]);  
aa = 1:4:length(x_time_smooth);
if size(m_PSTH(:,aa),  1) > 9
     for jj = 1 : size(m_PSTH(:,aa),  1) 
        plot(x_time_smooth(aa)-delay, m_PSTH(jj,aa), 'Color',  Rainbow1(jj, :));
        hold on;
    end
else
    for jj = 1 : size(m_PSTH(:,aa),  1) 
        plot(x_time_smooth(aa)-delay, m_PSTH(jj,aa), 'Color',  Rainbow(jj, :));
        hold on;
    end
end
...
plot([0 0],  [minpsth  maxpsth],  'r');
plot([0.7 0.7],  [minpsth  maxpsth],  'r');

index = find(x_time_smooth(aa)-delay<=0  &  x_time_smooth(aa)-delay >= -0.3);
...
ampl =0.5;
num_sigs = 3;
pos = ampl*0.5*(erf(4/duration*num_sigs/3*(t-duration/2)) + 1);
veloc = (maxpsth-spon)*diff(pos)/step +spon;

% plot(t(1:length(t)-1),veloc,'k-.');
% title('velocity (m/s)')
