...
function Heading_microstim(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, StartEventBin, StopEventBin, PATH, FILE);
...
axes('position',[0.2,0.3, 0.45,0.5] );
orient landscape;
...
for n = 1:length(unique_condition)
    for k = 1:length(unique_stim_type)      
        plot(unique_heading, correct_rate{n}(k,:), symbo{n,k},  xi, cum_gaussfit(psy_perf{n,k}, xi),  fitline{n,k} );
...
text(0,8.5, 'Stimtype   u0      sigma0     u1         sigma1             pu       psigma');

for n = 1:length(unique_condition)        
    for k = 1:length(unique_stim_type)  
        text(0,8-k, num2str(unique_stim_type(k)));  % non-microstim
        text(5+(n-1)*20,8-k,num2str(Bias_psy{n,k}) );
        text(15+(n-1)*20,8-k,num2str(Thresh_psy{n,k}) );
        text(40,8-k,num2str(P_bias(k)) ); % p value for bias shift
        text(50,8-k,num2str(P_slope(k)) ); % p value for slope change (threshold)
    end
end
axis off;
