...
filename = ['c:\data_analysis\optimal_integration\', FILE(1:9), '.mat'];
save(filename, 'fit_valid', 'unique_heading', 'unique_stim_type', 'unique_motion_coherence');
...
for c = 1:length(unique_motion_coherence) % different coherence level
    for k = 1:length(unique_stim_type)      
        ...
        plot(fit_valid{c,k}(:,1), fit_valid{c,k}(:,2), symbo{c,unique_stim_type(k)},  xi, cum_gaussfit(psy_perf{c,k}, xi),  fitline{c,unique_stim_type(k)} );
...
%        semilogy(psy_thresh_shift(k,:), f{k});
       ...
       plot(3:n+2,m_threshold, fitline{1,unique_stim_type(k)});
...
%         plot(3:n+2,psy_bias_shift(k,:), fitline{1,k});
       % semilogy(psy_thresh_shift(k,:), f{k});
        ...
        index = find(m_bias < -100)
        m_bias(index) = -100;
        plot(3:n+2,m_bias, fitline{1,unique_stim_type(k)});
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        hold on;
        xlabel('Repetition');  
        ylabel('Bias');
