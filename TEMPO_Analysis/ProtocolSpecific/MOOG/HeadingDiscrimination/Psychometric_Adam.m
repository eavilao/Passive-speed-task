...
% load(strcat(RESULTS_DIR,'Psychometric_adapt_',FILE(strfind(FILE,'m'):strfind(FILE,'c')-1),'.mat'),list(PRIORid).htb_file) %AZ I assume that the POST data is already calculated/saved
...
if ~isempty(channels) %get plexon event data - added by AZ 4/12/11
    ...
    try
        ...
        for ss=1:length(Succ_idx) %find the stop and start timestamps that came just before successful trial completion
            ...
            Sacc_diff=Succ_idx(ss)-find(Event_sv==7); Sacc_idx(ss)=Succ_idx(ss)-min(Sacc_diff(Sacc_diff>0)); %min distance of Saccade code (7) from successful code
            ...
            Sacc_del(ss)=Event_ts(Sacc_idx(ss))-Stop_ts(ss); %delay between the end of movement and choice selection (saccade). NB I use syncpulse to correct stim start/stop but the saccade event is realtime (no syncpulse correction) since it is input to tempo
        end
...
if use_tempo_spikes
    ...
    Sacc_del=(find(squeeze(data.event_data(channel,:,select_trials))==7)-find(squeeze(data.event_data(channel,:,select_trials))==5))/1000; %delay between end of movement and choice saccade (for PSTH around saccade)
    ...
    if ~isempty(channels) & isempty(strfind(plex_FILE,'m26c130')) %get the LFP data (except for one recording session which had a bug: fn and ts were extremely long? adfreq wrong?)
    ...
    for k = 1:length(unique_stim_type)
        ...
        for i = 1:length(unique_heading)
            ...
            for cell=1:cells
                ...
                raster{c,k,cell}(i,:,:) = PST{cell}(trials_select(i,:),:); % raster matrix (headings,trials,spikes). NB raster has half_guas + prePST*1000 before the actual response
                SACCdel{c,k,cell}(i,:)=Sacc_del(trials_select(i,:));
...
for c = 1:length(unique_motion_coherence) % different coherence level
    for k = 1:length(unique_stim_type)
        ...
        if valid_thresh{c,k} | ~exist('PRIORthresh95CI') | any(isnan(PRIORthresh95CI{k})) | unique_stim_type(k)==3, thresh_prior='';, else thresh_prior=sprintf('-cosine %f %f',PRIORthresh95CI{k}(1),PRIORthresh95CI{k}(4)); end %AZ if the fit will be based on one data point, use PREthresh as a 'prior'
        if sum(fit_use{c,k}(1:end,2)~=0 & fit_use{c,k}(1:end,2)~=1)>1 %if there are at least two numbers that are not zero or one
...
for k = 1:length(unique_stim_type) %NB assuming one level of coherence
    ...
    for cell=1:cells
        ...
        if max(resp{1,k,cell}(:))<1 %the cell did not fire. 2011_12_07 changed from 'if max(resp_mat{1,k,cell})<1' because could have low FR here but may respond to other modality (not corrected here, just made less likely)
            ...
            sdf_heading_mean=mean(pca_sdf,3); %can use a lowpass: sdf_heading_mean=filtfilt(d_FIR_pca.Numerator,1,sdf_heading_mean')';
%             %this calculates the PCA across ALL trials (unwanted since it includes variability within a heading, and we want variability across headings)           
%             [PCAcoeff_allTrial,PCAscore_allTrials] = pca(collapse_sdf','NumComponents',1);
%             eval(sprintf('%s.PCAcoeff_allTrials%s{cell}=PCAcoeff_allTrials;',var,stim{unique_stim_type(k)}))
%             eval(sprintf('%s.PCAscore_allTrials%s{cell}=reshape(PCAscore_allTrials,repetition,length(unique_heading))'';',var,stim{unique_stim_type(k)}))
%             eval(sprintf('[~,~,~,%s.PCAp_value_allTrials%s{cell}]= xy_regress(repmat(unique_heading(abs(unique_heading)<head_cut),[repetition 1]),reshape(%s.PCAscore_allTrials%s{cell}(abs(unique_heading)<head_cut,:),[],1));',var,stim{unique_stim_type(k)},var,stim{unique_stim_type(k)}))
            ...
            %using the PCA calculated across averaged headings, calculate the projection for each trial (for the first PCA only)
            if strcmp(PCAcentered,'on'), X=collapse_sdf'-repmat(PCAsdf_mean,size(collapse_sdf',1),1);
            else X=collapse_sdf'; end
            PCAscore_perTrial=reshape(X*PCAcoeff(:,1),repetition,length(unique_heading))';
            ...
            %caluculate PEAK and RMS p_values
...
try eval(sprintf('%s.waveformFreq=waveformFreq;',var)); end %I only save this for neuronal data
