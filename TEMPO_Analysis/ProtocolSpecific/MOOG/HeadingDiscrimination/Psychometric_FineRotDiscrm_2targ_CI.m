...
function Psychometric_FineRotDiscrm_2targ(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE);
...
one_repetition = (length(unique_heading)) * 1;


repetition = floor( length(heading)/one_repetition );

for ii = 1 : 1000
       
          
        cell_index = randsample(repetition,  repetition, 'true'); 
        clear fit_neuro_temp fit_data_psycho;
        for i = 1:length(unique_heading)
        
             tempindex = find((heading == unique_heading(i)) & (stim_type==unique_stim_type(k)));
             rightward_sel = find((heading == unique_heading(i)) & (stim_type==unique_stim_type(k))& (choice == RIGHT));
             rightward_trials = zeros(1, length(tempindex));
             ...
             if ~isempty(rightward_sel)
                   for jjj = 1 : length(rightward_sel)
                        temp1 = find(tempindex ==  rightward_sel(jjj));
                        rightward_trials(temp1) = 1;
                   end
             end         

             rightward_rate = sum(rightward_trials(cell_index)) / length(rightward_trials(cell_index));  
             fit_data_psycho(i, 1) = unique_heading(i);  
             fit_data_psycho(i, 2) = rightward_rate;
             fit_data_psycho(i, 3) = sum(trials_select); 
             fit_neuro_temp(i,3) = length(cell_index);
        end 
        wichman_psy = pfit(fit_data_psycho,'plot_opt','no plot','shape','cumulative gaussian','n_intervals',1,'sens',0,'compute_stats','false','verbose','false');
        Thresh_psy_perm(ii)  = wichman_psy.params.est(2);
        Bias_psy_perm(ii) = wichman_psy.params.est(1);
        
%          [bb, ss] = cum_gaussfit_max1(fit_data_psycho); 
%          Thresh_psy_perm(ii) =  ss;      

end
Thresh_psy_perm = sort(Thresh_psy_perm);
psy_dt = [Thresh_psy Thresh_psy_perm(25)    Thresh_psy_perm(975)];
addpath('Z:\Users\sheng\program\newtool');
writeexceldt(['Z:\Users\Courtney\yawtime\yaw'],FILE, psy_dt)
svfile = ['Z:\Users\Courtney\yawtime\'  FILE(1:end-4)];
eval(['save ' svfile  ' Thresh_psy_perm Bias_psy_perm  Thresh_psy  Bias_psy']);
