...
%-- draw only two target trial, Jing 10/19/2016
%-----------------------------------------------------------------------------------------------------------------------

function Psychometric_Gabor_2targ(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE);
...
for c = 1:length(unique_phase) 
    for k = 1:length(unique_stim_type)    
         for i = 1:length(unique_heading)
            trials_select =logical( (heading == unique_heading(i)) & (stim_type==unique_stim_type(k)) & (phase==unique_phase(c))& choice ~= 0 ) ;
         ...
         trials_all = find( ((heading < 0)|(heading > 0)) & (stim_type==unique_stim_type(k)) & (phase==unique_phase(c)) & choice ~= 0 ); %exclude 0 headings
         ...
         trials_rightward_overall = find( (heading > 0) & (choiceOverall==RIGHT) & (stim_type==unique_stim_type(k)) & (phase==unique_phase(c))) ;
         trials_leftward_overall  = find( (heading < 0) & (choiceOverall==LEFT) & (stim_type==unique_stim_type(k)) & (phase==unique_phase(c))) ;
         trials_all_overall = find( ((heading < 0)|(heading > 0)) & (stim_type==unique_stim_type(k)) & (phase==unique_phase(c))); %exclude 0 headings
...
text(10,8.5, '   u                           sigma                   correct rate             corr-rate-all                2targ-trial%');
...
for c = 1:length(unique_phase)  
    for k = 1:length(unique_stim_type)
        ...
        text(40,9-k-(c-1)*0.5,num2str(correct_proportion_overall(c,k)) );
        text(50,9-k-(c-1)*0.5,num2str(twoTarg_proportion(c,k)) );
    end
end
axis off;
