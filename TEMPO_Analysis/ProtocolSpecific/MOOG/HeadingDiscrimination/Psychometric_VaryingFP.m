...
function Psychometric_VaryingFP(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE);
...
for c = 1:length(unique_fix_x) % different FP location
    for k = 1:length(unique_stim_type)    
         for i = 1:length(unique_heading)
            trials_select =logical( (heading == unique_heading(i)) & (stim_type==unique_stim_type(k)) & (fix_x==unique_fix_x(c)) ) ;
         ...
         trials_all = find( ((heading < 0)|(heading > 0)) & (stim_type==unique_stim_type(k)) & (fix_x==unique_fix_x(c))); %exclude 0 headings
...
for c = 1:length(unique_fix_x)   %different FP location
    ...
    text(0.2+(c-1)*0.2,0.8, 'u                 sigma              correct rate');
    for k = 1:length(unique_stim_type)
