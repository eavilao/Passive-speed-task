...
%--	11/05/2012 Jing, 12/17/2012 changed to draw only two target trial
%-----------------------------------------------------------------------------------------------------------------------

function Psychometric_VaryingFP_2targ(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE);
...
for i= 1 : length(total_trials) 
    ...
    if (sum(events == IN_T1_WIN_CD) > 0)   %Jing 12/17/2012
...
correct_rate = [];
for c = 1:length(unique_fix_x) % different FP location
    for k = 1:length(unique_stim_type)    
         for i = 1:length(unique_heading)
            trials_select =logical( (heading == unique_heading(i)) & (stim_type==unique_stim_type(k)) & (fix_x==unique_fix_x(c))& choice ~= 0 ) ;
         ...
         trials_rightward = find( (heading > 0) & (choice==RIGHT) & (stim_type==unique_stim_type(k)) & (fix_x==unique_fix_x(c)) ) ;
         trials_leftward  = find( (heading < 0) & (choice==LEFT) & (stim_type==unique_stim_type(k)) & (fix_x==unique_fix_x(c)) ) ;
         trials_all = find( ((heading < 0)|(heading > 0)) & (stim_type==unique_stim_type(k)) & (fix_x==unique_fix_x(c)) & choice ~= 0 ); %exclude 0 headings
         ...
         %overall correct rate. 12/17/2012
         trials_rightward_overall = find( (heading > 0) & (choiceOverall==RIGHT) & (stim_type==unique_stim_type(k)) & (fix_x==unique_fix_x(c))) ;
         trials_leftward_overall  = find( (heading < 0) & (choiceOverall==LEFT) & (stim_type==unique_stim_type(k)) & (fix_x==unique_fix_x(c))) ;
         trials_all_overall = find( ((heading < 0)|(heading > 0)) & (stim_type==unique_stim_type(k)) & (fix_x==unique_fix_x(c))); %exclude 0 headings
         correct_proportion_overall(c,k) = (length(trials_rightward_overall)+length(trials_leftward_overall))/length(trials_all_overall);  
         twoTarg_proportion(c,k) = length(trials_all)/length(trials_all_overall);
    end
end
...
if overtimeplot == 1
    ...
    while EndTrial_shift <= EndTrial    
        ...
        for c = 1:length(unique_fix_x_shift)
            for k = 1:length(unique_condition_shift)
                for i = 1:length(unique_heading)
                    trials_shift =logical( (heading_shift == unique_heading(i)) & (condition_shift == unique_condition_shift(k)) & (fix_x_shift == unique_fix_x_shift(c))) ;
...
figure(2);
set(2,'Position', [200,100 1000,800], 'Name', 'Varying FP Heading Discrimination');
% fit data with cumulative gaussian and plot both raw data and fitted curve
legend_txt = [];
xi = min(unique_heading) : 0.1 : max(unique_heading);
lenc = length(unique_fix_x);
for c = 1:lenc 
    subplot('position', [0.07+(c-1)*0.3, 0.35, 0.24, 0.5]);
    for k = 1:length(unique_stim_type)      
        plot(unique_heading, fit_valid{c,k}(:,2), symbo{k},  xi, cum_gaussfit(psy_perf{c,k}, xi),  fitline{k} );
...
axes('position',[0.3,0.93, 0.6,0.05] );
xlim( [0,50] );
ylim( [2,10] );
text(0, 10, FILE);
...
for c = 1:length(unique_fix_x)   %different FP location
    axes('position',[0.03+(c-1)*0.26,0.82,0.24,0.15] );
    text(0.2+(c-1)*0.2,0.8, 'corr-rate      2targ-trial%');
    text(0.2+(c-1)*0.2,0.9, 'u                 sigma          2targ-corr rate');
    for k = 1:length(unique_stim_type)
        text((c-1)*0.2,0.75-k*0.1, num2str(unique_stim_type(k))); 
        text(0.1+(c-1)*0.2,0.75-k*0.1,num2str(Bias_psy{c,k}) );
        text(0.5+(c-1)*0.2,0.75-k*0.1,num2str(Thresh_psy{c,k}) );
        text(0.9+(c-1)*0.2,0.75-k*0.1,num2str(correct_proportion{c,k}) );
        text(0.1+(c-1)*0.2,0.65-k*0.1,num2str(correct_proportion_overall(c,k)) );
        text(0.5+(c-1)*0.2,0.65-k*0.1,num2str(twoTarg_proportion(c,k)) );
    end
    axis off;
end
...
if overtimeplot ==1
    for c = 1:length(unique_fix_x)   %different FP location
        ...
        axes('position',[0.07+(c-1)*0.3,0.05,0.24,0.1]);
        for k = 1:length(unique_stim_type)
