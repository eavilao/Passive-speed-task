...
case 'load data'
    ...
    set(ListHandle, 'String', listtext);
    if  isempty(strfind(lower(FILE),  'aff')) 
          [return_val, good_data, bad_data, newChans] = LoadTEMPOData(PATH,FILE);   
    else
          FILE  =  FILE(4:end);
          [return_val, good_data, bad_data, newChans, prooltoc11] =LoadTEMPODataALL(PATH,FILE)
    end
    ...
    if (return_val == -1)  %problem opening file
        ...
        for i=1:nchan
            ...
            if (DescriptionsExist)
                ...
                %JL added for plx data.03/06/2015
                %temp = sprintf('Recorded Spikes (Channel %d):  (%d events)', i, num_events);
                if isfield(good_data, 'spike_unit')
                    spsunit=good_data.spike_unit;
                    temp = sprintf('Recorded Spikes (Channel %d (sig %d,unit %d)):  (%d events)', i, spsunit(i,1), spsunit(i,2),num_events);               
                else
                    temp = sprintf('Recorded Spikes (Channel %d):  (%d events)', i, num_events);
                end % end 03/06/2015
            end
...
%     wavplay(wavread('Z:\LabTools\Matlab\TEMPO_Analysis\types.wav'),22050); % Announce file load complete JWN 01/26/07 
...
case 'analyze'
    ...
    FileHandle = findobj(gcbf, 'Tag', 'Filename');
    FILE = get(FileHandle, 'String');
    if  strcmp(lower(FILE(1:3)),  'aff') ==   1
        FILE = FILE(4:end);
    end
