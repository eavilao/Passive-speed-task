function results = analysespks(data,modality,prs)
...
for i=1:length(all_speeds)
    ...
    results.rate_pst(i,:) = hist(cell2mat({spk.tspk}'),data.stim.time)/(ntrls*prs.dt); % mean psth
    results.rate_pst_sig(i,:) = pst_sig(spk,data.stim.time,ntrls,prs); % compute variance of psth
...
if ~strcmp(modality,'null')
    ...
    alpha = 1-(1-prs.alpha)^(1/length(all_speeds));
    for i=1:length(all_speeds)
        spk = spks(speeds == all_speeds(i)); ntrls = length(spk);
        nspk(i,:) = [spk.nspk]; 
